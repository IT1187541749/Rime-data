...
if 参数!=nil && string.find(参数,"【【")!=nil && string.find(参数,"】】")!=nil then
 ...
 --print(候选)
 local 拆分语句目录=目录.."/data/拆分搜索"
 if string.sub(数据文件,1,#拆分语句目录)==拆分语句目录 then
  拆分搜索(数据文件,候选)
 end--if
 local 搜索语句目录=目录.."/data/普通搜索"
 if string.sub(数据文件,1,#搜索语句目录)==搜索语句目录 then
  搜索语句(数据文件,候选,string.sub(数据文件,#搜索语句目录+1,#搜索语句目录+1))
 end--if
